let url = $request.url;
const $ = new Env("fix-vvebo-user-timeline.js");
...
// URL处理函数
function handleUsersShow(url) {
    $.setval(getUid(url), "uid");
    $.done({});
}

function handleStatusesUserTimeline(url) {
    try {
        handleProfileStatusesTab($response.body)
    } catch {
...
function handleProfileStatusesTab(responseBody) {
...
function main(url) {
    if (url.includes("users/show")) {
        ...
        handleStatusesUserTimeline(url);
