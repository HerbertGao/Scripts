...
(async function launch() {
  ...
  if (m) handleProfile(m[1]);
})().catch(e => {
  $.msg("Blued", "错误", e.message || JSON.stringify(e))
}).finally(() => {
  $.done();
})

// 资料：解析 users 接口明文 JSON，转成资料卡片通知
function handleProfile(uidFromUrl) {
  ...
  // 自己的资料页请求不带查询参数；缓存自己的标签以便计算"我们的共同点"，且不弹通知
  if (url.indexOf('?') === -1) {
    $.setdata(String(d.uid || uidFromUrl), 'blued_self_uid');
    $.setdata($.toStr(tags) || '{}', 'blued_self_tags');
    return;
  }

  const title = `Blued ${d.name || ''} (uid ${d.uid || uidFromUrl})`;
  const line2 = [
    d.age,
  ...
  const iLike = tagNames([].concat(tags.love_type || [], tags.love_character || []));
  const common = commonPoints(tags);
...
function distanceText(loc) {
  if (!loc) return null;
  return String(loc).replace(/\s+/g, '');
}

function tagNames(arr) {
  if (!Array.isArray(arr) || !arr.length) return '';
  return arr.map(t => t && t.name).filter(Boolean).join('、');
}

// 我们的共同点：自己与对方在同一标签维度下的同名标签交集
function commonPoints(tags) {
  const selfTags = $.toObj($.getdata('blued_self_tags'), null);
  if (!selfTags) return '';
  const cats = ['type', 'character', 'love_type', 'love_character', 'i_want', 'hobbies', 'recreation', 'work', 'mbti'];
  const out = [];
  const seen = {};
  for (const c of cats) {
    const mine = {};
    (selfTags[c] || []).forEach(t => { if (t && t.name) mine[t.name] = 1; });
    (tags[c] || []).forEach(t => {
      const n = t && t.name;
      if (n && mine[n] && !seen[n]) { seen[n] = 1; out.push(n); }
    });
  }
  return out.join('、');
}
